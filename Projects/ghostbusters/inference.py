...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        for key, value in self.items():
            # (key, start, end)
            start = accumulated / total
            accumulated += value
            end = accumulated / total
            item = (key, start, end)
            my_list.append(item)

        s = random.random()
        for item in my_list:
            if s >= item[1] and s < item[2]:
                return item[0]
...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance: Union[float, None], pacmanPosition: tuple, ghostPosition: tuple, jailPosition):
        """
