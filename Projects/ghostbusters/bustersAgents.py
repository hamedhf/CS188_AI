    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        max_pos: Callable[[inference.DiscreteDistribution],
                          str] = lambda belief: belief.argMax()
        alive_positions = list(map(max_pos, livingGhostPositionDistributions))
        alive_positions = sorted(
            alive_positions, key=lambda p: self.distancer.getDistance(p, pacmanPosition))
        closest = alive_positions[0]
        # Actions.getSuccessor(pacmanPosition, action) returns the successor_position
        actions = sorted(legal, key=lambda action: self.distancer.getDistance(
            closest, Actions.getSuccessor(pacmanPosition, action)))
        return actions[0]
